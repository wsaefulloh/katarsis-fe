...
// core components
import CardBookingPartnership from "components/Cards/Admin/CardsBookingPartnership.js";
...
function PrivateBooking() {
  ...
  const getAll = async () => {
    const data = await fetchWrapper.get(`../api/admin/private-booking`);
    if (data) {
      setResult(data.data);
    }
  };
  ...
  const addData = async () => {
    setLoading(true);
    try {
      const response = await fetch(`../api/admin/private-booking`, {
        method: "POST",
        headers: {
    ...
    <>
      <Col xl="9" style={{ paddingLeft: "7.5px", paddingRight: "7.5px" }}>
        <Card style={{ height: "100%" }}>
            ...
            <Container>
              <div className="py-3">
                <Row className="align-items-center">
                  <Col>
                    <h3 className="mb-0">List Booking & Partership Menu</h3>
                  </Col>
                  <Button
          ...
          <Container>
            {result.map((val) => {
              return (
                <CardBookingPartnership
                  id={val.id}
                  name_private={val.name_private}
                  url_image={val.url_imageOrigin}
                  url_email={val.url_email}
      ...
      >
        <div className="justify-content-center text-center p-4">
          ...
          ></img>
          <h3 className="m-0 mb-3 p-0">Tambah Booking & Partership Menu</h3>
          <Form>
            <div className="form-row">
              <Col className="mb-3 p-0 text-left ">
                <h5 className="pl-1 mb-1">Nama Partnership</h5>
                <Input
                  type="text"
                  defaultValue={`${dataValue.name_private}`}
                  onChange={(e) => {
                    setDataValue({
                      ...dataValue,
                      name_private: `${e.target.value}`,
                    });
                  }}
                />
              </Col>
            ...
            <div className="form-row">
              <Col className="mb-3 p-0 text-left ">
                <h5 className="pl-1 mb-1">
                  URL Google Drive Image Partership (760 x 400)
                </h5>
                <Input
                  type="text"
          ...
          <Form>
            <div className="form-row">
              <Col className="mb-3 p-0 text-left ">
                <h5 className="pl-1 mb-1">Email Addres for Partership</h5>
                <Input
                  type="text"
                  defaultValue={`${dataValue.url_email}`}
                  onChange={(e) => {
                    setDataValue({
                      ...dataValue,
                      url_email: `${e.target.value}`,
                    });
                  }}
                />
              </Col>
          ...
          <Form>
            <div className="form-row">
              <Col className="mb-3 p-0 text-left ">
                <h5 className="pl-1 mb-1">Description for Partership</h5>
                <Input
                  type="text"
...
PrivateBooking.layout = HomeLayoutAdmin;
