...
function SocialLinks() {
  ...
  const getSomeRequest = async (request) => {
    const data = await fetchWrapper.get(`../api/links/${request}`);
    if (data) {
  ...
  useEffect(() => {
    getSomeRequest("get-cta-partnership");
  }, []);
    ...
    <>
            ...
            <Container>
              <div className="py-3">
                <Row className="align-items-center">
                  <Col>
                    <h3 className="mb-0">CTA Workflow</h3>
                  </Col>
                </Row>
                    ...
                    <Form>
                      <div className="form-row">
                        <Col className="mb-3 p-0 text-left ">
                          <div className="mb-1">Title CTA</div>

                          <Input
                            disabled
                            type="text"
                            defaultValue={`${resultLinkedin.title}`}
                          />
                        </Col>
                      </div>
                    </Form>
      ...
      >
        <div className="justify-content-center text-center p-4">
          ...
          ></img>
          <h3 className="m-0 mb-3 p-0">Update CTA Workflow</h3>
          <Form>
            <div className="form-row">
              <Col className="mb-3 p-0 text-left ">
                <h5>Title CTA Workflow</h5>
                <Input
                  type="text"
                  defaultValue={`${resultLinkedin.title}`}
                  onChange={(e) => {
          ...
          <Form>
            <div className="form-row">
              <Col className="mb-3 p-0 text-left ">
                <h5>URL CTA Workflow</h5>
                <div>Gunakan (mailto:xxx@mail.com) untuk email</div>
                <div>Gunakan (https://wa.me/628XXX) untuk whatsapp</div>
                <Input
                  type="text"
                  defaultValue={`${resultLinkedin.url}`}
                  onChange={(e) => {
          ...
          <Button
            ...
            onClick={() => {
              let data = {
                ...
                title:
                  dataLinkedin.title.length == 0
                    ? resultLinkedin.title
                    : dataLinkedin.title,
                url:
