import React, { useEffect, useState } from "react";

// reactstrap components
import {
  Button,
  Container,
  Row,
  Col,
  Modal,
  Card,
  Spinner,
  Form,
  Input,
} from "reactstrap";
// layout for this page
import HomeLayoutAdmin from "layouts/Admin.js";
// core components
import { fetchWrapper } from "../../helpers/fetch-wrapper";

import "../../assets/css/main/main.module.css";
import Swal from "sweetalert2";
import { useRouter } from "next/router";

function SocialLinks() {
  const [modalOpen1, setModalOpen1] = useState(false);
  const [modalOpen2, setModalOpen2] = useState(false);
  const [modalOpen3, setModalOpen3] = useState(false);
  const [modalOpen4, setModalOpen4] = useState(false);

  const [loading, setLoading] = useState(false);
  const [data1, setData1] = useState({
    id: "",
    type: "",
    title: "",
    url: "",
    url_images: "",
    description: "",
  });
  const [result1, setResult1] = useState([]);

  const [process1, setProcess1] = useState({});
  const [process2, setProcess2] = useState({});
  const [process3, setProcess3] = useState({});

  const router = useRouter();

  const getAllProcess = async () => {
    const data = await fetchWrapper.get(`../api/content/get-workflow`);
    if (data) {
      let realResult = data.data;
      setResult1(realResult[0]);
    }
  };

  const getAllProcessWorkflow = async () => {
    const data = await fetchWrapper.get(`../api/content/get-process-workflow`);
    if (data) {
      let realResult = data.data;
      setProcess1(realResult[0]);
      setProcess2(realResult[1]);
      setProcess3(realResult[2]);
    }
  };

  const updateSocialLink = async (dataUpdate) => {
    setLoading(true);
    try {
      const response = await fetch(`../api/admin/content`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
        },
        body: JSON.stringify(dataUpdate),
      });
      const data = await response.json();
      if (data) {
        if (data.statusCode != 201) {
          Swal.fire("FAILED", `Data gagal diupdate`, "error");
          setLoading(false);
        } else {
          await Swal.fire("OK", `Data berhasil diupdate`, "success");
          router.reload(window.location.pathname);
        }
      }
      setLoading(false);
    } catch (error) {
      Swal.fire("FAILED", "Data gagal di proses", "error");
      setLoading(false);
    }
  };

  useEffect(() => {
    getAllProcess();
    getAllProcessWorkflow();
  }, []);

  return (
    <>
      <Col xl="9" style={{ paddingLeft: "7.5px", paddingRight: "7.5px" }}>
        <Card style={{ height: "100%" }}>
          <Card>
            <Container>
              <div className="py-3">
                <Row className="align-items-center">
                  <Col>
                    <h3 className="mb-0">Main Content Workflow</h3>
                  </Col>
                </Row>
                <Row className="align-items-center mt-2">
                  <Col>
                    <Form>
                      <div className="form-row">
                        <Col className="mb-3 p-0 text-left ">
                          <div className="mb-1">Title Workflow</div>

                          <Input
                            disabled
                            type="text"
                            defaultValue={`${result1.title}`}
                          />
                        </Col>
                      </div>
                    </Form>
                  </Col>
                </Row>
                <Row className="align-items-center mt-2">
                  <Col>
                    <Form>
                      <div className="form-row">
                        <Col className="mb-3 p-0 text-left ">
                          <div className="mb-1">Description Workflow</div>

                          <Input
                            disabled
                            type="text"
                            defaultValue={`${result1.description}`}
                          />
                        </Col>
                      </div>
                    </Form>
                  </Col>
                </Row>
                <Button
                  className="m-1 border-0 py-1 px-3"
                  style={{
                    color: "#ffffff",
                    backgroundColor: "#13678a",
                    borderRadius: "5px",
                    fontSize: "10px",
                  }}
                  type="button"
                  onClick={() => {
                    setModalOpen1(!modalOpen1);
                  }}
                >
                  <span>Update</span>
                </Button>
              </div>
            </Container>
          </Card>

          <Card className="mb-2">
            <div className="py-3 px-2">
              <Row className="align-items-center">
                <Col>
                  <span className="mb-0">Keterangan Line 1</span>
                </Col>
                <Col>
                  <Form>
                    <div className="form-row">
                      <Col className="p-0 text-left ">
                        <Input
                          disabled="true"
                          type="text"
                          defaultValue={`${process1.description}`}
                        />
                      </Col>
                    </div>
                  </Form>
                </Col>

                <Button
                  className="m-1 border-0 py-1 px-3"
                  style={{
                    color: "#ffffff",
                    backgroundColor: "#13678a",
                    borderRadius: "5px",
                    fontSize: "10px",
                  }}
                  type="button"
                  onClick={() => {
                    setModalOpen2(!modalOpen2);
                  }}
                >
                  <span>Update</span>
                </Button>
              </Row>
            </div>
          </Card>
          <Card className="mb-2">
            <div className="py-3 px-2">
              <Row className="align-items-center">
                <Col>
                  <span className="mb-0">Keterangan Line 2</span>
                </Col>
                <Col>
                  <Form>
                    <div className="form-row">
                      <Col className="p-0 text-left ">
                        <Input
                          disabled="true"
                          type="text"
                          defaultValue={`${process2.description}`}
                        />
                      </Col>
                    </div>
                  </Form>
                </Col>

                <Button
                  className="m-1 border-0 py-1 px-3"
                  style={{
                    color: "#ffffff",
                    backgroundColor: "#13678a",
                    borderRadius: "5px",
                    fontSize: "10px",
                  }}
                  type="button"
                  onClick={() => {
                    setModalOpen3(!modalOpen3);
                  }}
                >
                  <span>Update</span>
                </Button>
              </Row>
            </div>
          </Card>
          <Card className="mb-2">
            <div className="py-3 px-2">
              <Row className="align-items-center">
                <Col>
                  <span className="mb-0">Keterangan Line 3</span>
                </Col>
                <Col>
                  <Form>
                    <div className="form-row">
                      <Col className="p-0 text-left ">
                        <Input
                          disabled="true"
                          type="text"
                          defaultValue={`${process3.description}`}
                        />
                      </Col>
                    </div>
                  </Form>
                </Col>

                <Button
                  className="m-1 border-0 py-1 px-3"
                  style={{
                    color: "#ffffff",
                    backgroundColor: "#13678a",
                    borderRadius: "5px",
                    fontSize: "10px",
                  }}
                  type="button"
                  onClick={() => {
                    setModalOpen4(!modalOpen4);
                  }}
                >
                  <span>Update</span>
                </Button>
              </Row>
            </div>
          </Card>
        </Card>
      </Col>

      <Modal
        toggle={() => setModalOpen1(!modalOpen1)}
        isOpen={modalOpen1}
        centered
        fade={true}
        size="md"
      >
        <div className="justify-content-center text-center p-4">
          <img
            onClick={() => {
              setModalOpen1(false);
            }}
            style={{
              position: "absolute",
              top: "8px",
              right: "8px",
              width: "35px",
              cursor: "pointer",
            }}
            src="data:image/png;base64,"
          ></img>
          <h3 className="m-0 mb-3 p-0">Update Workflow</h3>
          <Form>
            <div className="form-row">
              <Col className="mb-3 p-0 text-left ">
                <h5>Title Workflow</h5>
                <Input
                  type="text"
                  defaultValue={`${result1.title}`}
                  onChange={(e) => {
                    setData1({
                      ...data1,
                      title: `${e.target.value}`,
                    });
                  }}
                />
              </Col>
            </div>
            <div className="form-row">
              <Col className="mb-3 p-0 text-left ">
                <h5>Description Workflow</h5>
                <Input
                  type="text"
                  defaultValue={`${result1.description}`}
                  onChange={(e) => {
                    setData1({
                      ...data1,
                      description: `${e.target.value}`,
                    });
                  }}
                />
              </Col>
            </div>
          </Form>

          <Button
            color="secondary"
            style={{
              color: "#ffffff",
              backgroundColor: "#13678a",
              maxWidth: "150px",
            }}
            className="border-0"
            type="button"
            onClick={() => {
              let data = {
                id: result1.id,
                type: result1.type,
                title: data1.title.length == 0 ? result1.title : data1.title,
                description:
                  data1.description.length == 0
                    ? result1.description
                    : data1.description,
                url_images:
                  data1.url_images.length == 0
                    ? result1.url_images
                    : data1.url_images,
              };
              updateSocialLink(data);
            }}
          >
            {loading == true ? (
              <>
                <div className="py-1 text-center align-content-center align-items-center">
                  <Spinner
                    as="span"
                    animation="grow"
                    size="sm"
                    role="status"
                    aria-hidden="true"
                  />
                </div>
              </>
            ) : (
              <>Update</>
            )}
          </Button>
        </div>
      </Modal>

      <Modal
        toggle={() => setModalOpen2(!modalOpen2)}
        isOpen={modalOpen2}
        centered
        fade={true}
        size="md"
      >
        <div className="justify-content-center text-center p-4">
          <img
            onClick={() => {
              setModalOpen2(false);
            }}
            style={{
              position: "absolute",
              top: "8px",
              right: "8px",
              width: "35px",
              cursor: "pointer",
            }}
            src="data:image/png;base64,"
          ></img>
          <h3 className="m-0 mb-3 p-0">Update Process Workflow</h3>
          <Form>
            <div className="form-row">
              <Col className="mb-3 p-0 text-left ">
                <h5>Description Workflow</h5>
                <Input
                  type="text"
                  defaultValue={`${process1.description}`}
                  onChange={(e) => {
                    setProcess1({
                      ...process1,
                      description: `${e.target.value}`,
                    });
                  }}
                />
              </Col>
            </div>
          </Form>

          <Button
            color="secondary"
            style={{
              color: "#ffffff",
              backgroundColor: "#13678a",
              maxWidth: "150px",
            }}
            className="border-0"
            type="button"
            onClick={() => {
              let data = {
                id: process1.id,
                type: process1.type,
                title: process1.title,
                description: process1.description,
                url_images: process1.url_images,
              };
              updateSocialLink(data);
            }}
          >
            {loading == true ? (
              <>
                <div className="py-1 text-center align-content-center align-items-center">
                  <Spinner
                    as="span"
                    animation="grow"
                    size="sm"
                    role="status"
                    aria-hidden="true"
                  />
                </div>
              </>
            ) : (
              <>Update</>
            )}
          </Button>
        </div>
      </Modal>

      <Modal
        toggle={() => setModalOpen3(!modalOpen3)}
        isOpen={modalOpen3}
        centered
        fade={true}
        size="md"
      >
        <div className="justify-content-center text-center p-4">
          <img
            onClick={() => {
              setModalOpen3(false);
            }}
            style={{
              position: "absolute",
              top: "8px",
              right: "8px",
              width: "35px",
              cursor: "pointer",
            }}
            src="data:image/png;base64,"
          ></img>
          <h3 className="m-0 mb-3 p-0">Update Process Workflow</h3>
          <Form>
            <div className="form-row">
              <Col className="mb-3 p-0 text-left ">
                <h5>Description Workflow</h5>
                <Input
                  type="text"
                  defaultValue={`${process2.description}`}
                  onChange={(e) => {
                    setProcess2({
                      ...process2,
                      description: `${e.target.value}`,
                    });
                  }}
                />
              </Col>
            </div>
          </Form>

          <Button
            color="secondary"
            style={{
              color: "#ffffff",
              backgroundColor: "#13678a",
              maxWidth: "150px",
            }}
            className="border-0"
            type="button"
            onClick={() => {
              let data = {
                id: process2.id,
                type: process2.type,
                title: process2.title,
                description: process2.description,
                url_images: process2.url_images,
              };
              updateSocialLink(data);
            }}
          >
            {loading == true ? (
              <>
                <div className="py-1 text-center align-content-center align-items-center">
                  <Spinner
                    as="span"
                    animation="grow"
                    size="sm"
                    role="status"
                    aria-hidden="true"
                  />
                </div>
              </>
            ) : (
              <>Update</>
            )}
          </Button>
        </div>
      </Modal>

      <Modal
        toggle={() => setModalOpen4(!modalOpen4)}
        isOpen={modalOpen4}
        centered
        fade={true}
        size="md"
      >
        <div className="justify-content-center text-center p-4">
          <img
            onClick={() => {
              setModalOpen4(false);
            }}
            style={{
              position: "absolute",
              top: "8px",
              right: "8px",
              width: "35px",
              cursor: "pointer",
            }}
            src="data:image/png;base64,"
          ></img>
          <h3 className="m-0 mb-3 p-0">Update Process Workflow</h3>
          <Form>
            <div className="form-row">
              <Col className="mb-3 p-0 text-left ">
                <h5>Description Workflow</h5>
                <Input
                  type="text"
                  defaultValue={`${process3.description}`}
                  onChange={(e) => {
                    setProcess3({
                      ...process3,
                      description: `${e.target.value}`,
                    });
                  }}
                />
              </Col>
            </div>
          </Form>

          <Button
            color="secondary"
            style={{
              color: "#ffffff",
              backgroundColor: "#13678a",
              maxWidth: "150px",
            }}
            className="border-0"
            type="button"
            onClick={() => {
              let data = {
                id: process3.id,
                type: process3.type,
                title: process3.title,
                description: process3.description,
                url_images: process3.url_images,
              };
              updateSocialLink(data);
            }}
          >
            {loading == true ? (
              <>
                <div className="py-1 text-center align-content-center align-items-center">
                  <Spinner
                    as="span"
                    animation="grow"
                    size="sm"
                    role="status"
                    aria-hidden="true"
                  />
                </div>
              </>
            ) : (
              <>Update</>
            )}
          </Button>
        </div>
      </Modal>
    </>
  );
}

SocialLinks.layout = HomeLayoutAdmin;

export default SocialLinks;
