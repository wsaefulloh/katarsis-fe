...
// core components
import CardTicketPlatform from "components/Cards/Admin/CardsTicketPlatform.js";
...
function TicketPlatform() {
  ...
  const getAll = async () => {
    const data = await fetchWrapper.get(`../api/admin/ticket-platform`);
    if (data) {
      setResult(data.data);
    }
  };
  ...
  const addData = async () => {
    setLoading(true);
    try {
      const response = await fetch(`../api/admin/ticket-platform`, {
        method: "POST",
        headers: {
            ...
            <Container>
              <div className="py-3">
                <Row className="align-items-center">
                  <Col>
                    <h3 className="mb-0">List Ticket Platform</h3>
                  </Col>
                  <Button
          ...
          <Container>
            {result.map((val) => {
              return (
                <CardTicketPlatform
                  id={val.id}
                  name_ticket={val.name_ticket}
                  url_image={val.url_imageOrigin}
                  url_ticket={val.url_ticket}
                />
              );
            })}
          </Container>
      ...
      >
        <div className="justify-content-center text-center p-4">
          ...
          <Form>
            <div className="form-row">
              <Col className="mb-3 p-0 text-left ">
                <h5 className="pl-1 mb-1">Nama Ticket Platform</h5>
                <Input
                  type="text"
                  defaultValue={`${dataValue.name_ticket}`}
                  onChange={(e) => {
                    setDataValue({
                      ...dataValue,
                      name_ticket: `${e.target.value}`,
                    });
                  }}
                />
              </Col>
            ...
            <div className="form-row">
              <Col className="mb-3 p-0 text-left ">
                <h5 className="pl-1 mb-1">
                  URL Google Drive Image Ticket Platform
                </h5>
                <Input
                  type="text"
          ...
          <Form>
            <div className="form-row">
              <Col className="mb-3 p-0 text-left ">
                <h5 className="pl-1 mb-1">URL Ticket Platform</h5>
                <Input
                  type="text"
                  defaultValue={`${dataValue.url_ticket}`}
                  onChange={(e) => {
                    setDataValue({
                      ...dataValue,
                      url_ticket: `${e.target.value}`,
                    });
                  }}
                />
              </Col>
...
TicketPlatform.layout = HomeLayoutAdmin;
