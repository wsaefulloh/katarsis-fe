...
function HomeStatus() {
  ...
  const getSomeRequest = async (request) => {
    ...
    if (data) {
      ...
      if (request == "home-status") {
        setResultStatus(realResult[0]);
      } else if (request == "home-background") {
  ...
  useEffect(() => {
    getSomeRequest("home-background");
    getSomeRequest("home-status");
  }, []);
      ...
      >
        <div className="justify-content-center text-center p-4">
          ...
          <Form>
            <div className="form-row">
              <Col className="mb-3 p-0 text-left ">
                <h5>Status Page Home Page (active or nonactive)</h5>
                <Input
                  type="text"
...
HomeStatus.layout = HomeLayoutAdmin;
