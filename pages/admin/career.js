import React, { useEffect, useState } from "react";

// reactstrap components
import {
  Button,
  Container,
  Row,
  Col,
  Modal,
  Card,
  Spinner,
  Form,
  Input,
  UncontrolledDropdown,
  DropdownToggle,
  DropdownMenu,
  DropdownItem,
} from "reactstrap";
// layout for this page
import HomeLayoutAdmin from "layouts/Admin.js";
// core components
import CardsDepartement from "../../components/Admin/CardsDepartement";
import { fetchWrapper } from "../../helpers/fetch-wrapper";

import "../../assets/css/main/main.module.css";
import Swal from "sweetalert2";
import { useRouter } from "next/router";

function Career() {
  const [loading, setLoading] = useState(false);
  const router = useRouter();
  const [modalOpen1, setModalOpen1] = useState(false);
  const [dataValue1, setDataValue1] = useState({
    title_departement: "",
    description_departement: "",
  });
  const [result1, setResult1] = useState([]);

  const getAll1 = async () => {
    const data = await fetchWrapper.get(`../api/get-departement`);
    if (data) {
      setResult1(data.data);
    }
  };

  const addData1 = async () => {
    setLoading(true);
    try {
      const response = await fetch(`../api/admin/departement`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
        body: JSON.stringify(dataValue1),
      });
      const data = await response.json();
      if (data) {
        if (data.statusCode != 201) {
          Swal.fire("FAILED", "Lengkapi semua data!", "error");
          setLoading(false);
        } else {
          await Swal.fire("OK", "Data berhasil ditambahkan", "success");
          router.reload(window.location.pathname);
        }
      }
      setLoading(false);
    } catch (error) {
      Swal.fire("FAILED", "Data gagal di proses", "error");
      setLoading(false);
    }
  };

  useEffect(() => {
    getAll1();
  }, []);

  return (
    <>
      <Col xl="9" style={{ paddingLeft: "7.5px", paddingRight: "7.5px" }}>
        <Card style={{ height: "100%" }}>
          <Card>
            <Container>
              <div className="py-3">
                <Row className="align-items-center">
                  <Col>
                    <h3 className="mb-0">{`List Departement dan Job yang dibuka`}</h3>
                  </Col>
                  <Button
                    className="m-1 border-0 py-1 px-3"
                    style={{
                      color: "#ffffff",
                      backgroundColor: "#000000",
                      borderRadius: "5px",
                    }}
                    type="button"
                    onClick={() => {
                      setModalOpen1(!modalOpen1);
                    }}
                  >
                    <span>Tambah</span>
                  </Button>
                </Row>
              </div>
            </Container>
          </Card>

          <Container>
            {result1.map((val) => {
              return (
                <CardsDepartement
                  id={val.id}
                  title={val.title_departement}
                  description={val.description_departement}
                />
              );
            })}
          </Container>
        </Card>
      </Col>
      <Modal
        toggle={() => setModalOpen1(!modalOpen1)}
        isOpen={modalOpen1}
        centered
        fade={true}
        size="md"
      >
        <div className="justify-content-center text-center p-4">
          <img
            onClick={() => {
              setModalOpen1(false);
            }}
            style={{
              position: "absolute",
              top: "8px",
              right: "8px",
              width: "35px",
              cursor: "pointer",
            }}
            src="data:image/png;base64,"
          ></img>
          <h3 className="m-0 mb-3 p-0">Tambah Departemen Career</h3>

          <Form>
            <div className="form-row">
              <Col className="mb-3 p-0 text-left ">
                <h5 className="pl-1 mb-1">Title Departement</h5>
                <Input
                  type="text"
                  defaultValue={`${dataValue1.title_departement}`}
                  onChange={(e) => {
                    setDataValue1({
                      ...dataValue1,
                      title_departement: `${e.target.value}`,
                    });
                  }}
                />
              </Col>
            </div>
          </Form>

          <Form>
            <div className="form-row">
              <Col className="mb-3 p-0 text-left ">
                <h5 className="pl-1 mb-1">Deskripsi Departement Career</h5>
                <Input
                  type="text"
                  defaultValue={`${dataValue1.description_departement}`}
                  onChange={(e) => {
                    setDataValue1({
                      ...dataValue1,
                      description_departement: `${e.target.value}`,
                    });
                  }}
                />
              </Col>
            </div>
          </Form>

          <Button
            color="secondary"
            style={{
              color: "#ffffff",
              backgroundColor: "#000000",
              maxWidth: "150px",
            }}
            className="border-0"
            type="button"
            onClick={() => {
              addData1();
            }}
          >
            {loading == true ? (
              <>
                <div className="py-1 text-center align-content-center align-items-center">
                  <Spinner
                    as="span"
                    animation="grow"
                    size="sm"
                    role="status"
                    aria-hidden="true"
                  />
                </div>
              </>
            ) : (
              <>Tambah</>
            )}
          </Button>
        </div>
      </Modal>
      ;
    </>
  );
}

Career.layout = HomeLayoutAdmin;

export default Career;
